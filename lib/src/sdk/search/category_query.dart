...
class CategoryQueryArea {
  ...
  /// Constructs a new instance of this class from provided parameters.
  ///
  /// The given corridor defines the area that will be used in the search query.
  ///
  /// When used with SearchEngine, the polyline is compressed and sent.
  /// More complex polylines with large amounts of coordinates and with smaller
  /// half-width may have the less relevant part removed, such as the one far away from the
  ...
  ///
  @Deprecated("Will be removed in v4.20.0. Instead, please use CategoryQuery_Area.withCorridorAndCenter.")
  ...
  ///
  factory CategoryQueryArea.withCorridorAndCenter(GeoCorridor corridorArea, GeoCoordinates areaCenter) => $prototype.withCorridorAndCenter(corridorArea, areaCenter);
  @override
  bool operator ==(Object other) {
    if (identical(this, other)) return true;
...
class CategoryQueryArea$Impl {
  ...
  CategoryQueryArea withCorridorAndCenter(GeoCorridor corridorArea, GeoCoordinates areaCenter) {
    final _withCorridorAndCenterFfi = __lib.catchArgumentError(() => __lib.nativeLibrary.lookupFunction<Pointer<Void> Function(Int32, Pointer<Void>, Pointer<Void>), Pointer<Void> Function(int, Pointer<Void>, Pointer<Void>)>('here_sdk_sdk_search_CategoryQuery_Area_make__GeoCorridor_GeoCoordinates'));
    final _corridorAreaHandle = sdkCoreGeocorridorToFfi(corridorArea);
    final _areaCenterHandle = sdkCoreGeocoordinatesToFfi(areaCenter);
    final __resultHandle = _withCorridorAndCenterFfi(__lib.LibraryContext.isolateId, _corridorAreaHandle, _areaCenterHandle);
    sdkCoreGeocorridorReleaseFfiHandle(_corridorAreaHandle);
    sdkCoreGeocoordinatesReleaseFfiHandle(_areaCenterHandle);
    try {
      return sdkSearchCategoryqueryAreaFromFfi(__resultHandle);
