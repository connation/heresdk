...
class ConsentEngine implements Consent {
  ...
  final ConsentInternal _consent;

  ConsentEngine() : this._withConsentInternal(ConsentInternal.fromSharedSdkNativeEngine());

  ConsentEngine.withSdkEngine(SDKNativeEngine sdkEngine)
      : this._withConsentInternal(ConsentInternal.fromSdkNativeEngine(sdkEngine));

  ConsentEngine._withConsentInternal(this._consent);
  ...
  /// Polish, Portuguese, Portuguese (Brazil), Romanian (Romania), Russian, Slovak (Slovakia), Serbian,
  /// Swedish, Thai, Turkish, Ukrainian (Ukraine), Vietnamese, Chinese (Simplified Han,China), Chinese
  /// (Simplified Han,Hong Kong), Chinese (Traditional Han,Taiwan).
