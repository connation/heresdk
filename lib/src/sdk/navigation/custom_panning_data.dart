...
/// allowing the customization of the spatial audio trajectories.
///
...
/// When a double notification maneuver is triggered, the parameters
/// [CustomPanningData.initialAzimuthInDegrees] or [CustomPanningData.sweepAzimuthInDegrees]
/// will not be considered when calling [SpatialAudioCuePanning.startAngularPanning] or [SpatialManeuverAudioCuePanning.startPanning].
/// Instead, the audio cue is spatialized considering the action of both maneuvers, for example,
...
/// Note that the estimation for playing both audio cues could be not fully accurate and therefore
...
class CustomPanningData {
  ...
  /// [estimatedAudioCueDuration] Customized estimated duration for playing the audio cue on the selected TTS Engine.
  /// When not used, HERE SDK's estimation will be used instead.
  /// [initialAzimuthInDegrees] Initial desired angular position of the upcoming audio cue. For example, for a maneuver such
  /// as "Turn right on" (`ManeuverAction.RightTurn`) we want to create a spatial audio arc
  /// from the front to the right, mimicking the maneuver geometry. In this case,
  /// it is good practice to start the trajectory from an initial azimuth that is slightly located
  /// on the opposite direction of the maneuver (e.g. slightly starting from "front-left")
  ...
  /// This azimuth value is needed to set the position of the audio renderer before starting
  /// to play the audio cue to avoid unwanted audio "jumps".
  /// [sweepAzimuthInDegrees] Sweep angle of the upcoming audio cue. For example, for a maneuver such as "Turn right on"
  /// (i.e. `ManeuverAction.RightTurn`),
  /// within an `initial_azimuth_in_degrees` of -5 degrees, we want to create a spatial audio arc
  /// trajectory from the front to the right, mimicking the maneuver geometry.
  /// In this case, the desired final angle would be +90 degrees, and therefore, a sweep angle of
