...
class RouteProgress {
  ...
  /// [sectionIndex] Index of the [Section] in the route.
  /// Note that this section index does not point to the current [SectionProgress]
  /// but to the route [Section] that you can access via [NavigatorInterface.route]
  /// and [Route.sections].

  /// [spanIndex] Index of the [Span] in the route section.

  /// [sectionProgress] The progress for each [Section] from the current one to the last one.
  /// Note that the progress information is accumulated successively, therefore information relative
  /// to the final destination is in the last item of the list. The list is guaranteed to be non-empty.

  /// [maneuverProgress] The progress for next and next-next maneuvers (see [Maneuver]). Note that the list
  /// can contain at maximum two items (for next and next-next maneuvers) and one or zero when approaching the
  /// destination.

  RouteProgress(this.sectionIndex, this.spanIndex, this.sectionProgress, this.maneuverProgress);
  @override
  ...
  int get hashCode {
    ...
    result = 31 * result + spanIndex.hashCode;
...
final _sdkNavigationRouteprogressCreateHandle = __lib.catchArgumentError(() => __lib.nativeLibrary.lookupFunction<
    Pointer<Void> Function(Int32, Int32, Pointer<Void>, Pointer<Void>),
...
final _sdkNavigationRouteprogressGetFieldspanIndex = __lib.catchArgumentError(() => __lib.nativeLibrary.lookupFunction<
    Int32 Function(Pointer<Void>),
    int Function(Pointer<Void>)
  >('here_sdk_sdk_navigation_RouteProgress_get_field_spanIndex'));
...
Pointer<Void> sdkNavigationRouteprogressToFfi(RouteProgress value) {
  ...
  final _spanIndexHandle = (value.spanIndex);
  ...
  final _result = _sdkNavigationRouteprogressCreateHandle(_sectionIndexHandle, _spanIndexHandle, _sectionProgressHandle, _maneuverProgressHandle);
...
RouteProgress sdkNavigationRouteprogressFromFfi(Pointer<Void> handle) {
  ...
  final _spanIndexHandle = _sdkNavigationRouteprogressGetFieldspanIndex(handle);
