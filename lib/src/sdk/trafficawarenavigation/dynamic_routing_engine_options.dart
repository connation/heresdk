...
class DynamicRoutingEngineOptions {
  ...
  /// A value of 0 will be treated as `null` meaning no event will be sent.
  ...
  /// [minTimeDifference] The minimum time difference, before notifying the [DynamicRoutingListener].
  /// To get notified, the following check must be true:
  /// oldEstimatedTimeOfArrival - newEstimatedTimeOfArrival > [DynamicRoutingEngineOptions.minTimeDifference].
  /// A value of 0 will be treated as `null` meaning no event will be sent.
  /// In order to receive events the difference needs to be greater than 0.
  ...
  DynamicRoutingEngineOptions._(this.minTimeDifferencePercentage, this.minTimeDifference, this.pollInterval);
  /// Creates an instance of this class.
  DynamicRoutingEngineOptions()
      : minTimeDifferencePercentage = null, minTimeDifference = null, pollInterval = const Duration(minutes: 15);
  @override
  bool operator ==(Object other) {
    if (identical(this, other)) return true;
    if (other is! DynamicRoutingEngineOptions) return false;
    DynamicRoutingEngineOptions _other = other;
    return minTimeDifferencePercentage == _other.minTimeDifferencePercentage &&
...
Pointer<Void> sdkTrafficawarenavigationDynamicroutingengineoptionsToFfi(DynamicRoutingEngineOptions value) {
  ...
  final _result = _sdkTrafficawarenavigationDynamicroutingengineoptionsCreateHandle(_minTimeDifferencePercentageHandle, _minTimeDifferenceHandle, _pollIntervalHandle);
