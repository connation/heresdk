...
class Region {
  ...
  /// [parentId] Parent region identifier, not expose to platforms.

  /// [name] Name of region. Language is determined by the requested [LanguageCode]. By default,
  /// it is in [LanguageCode.enUs].

  /// [sizeOnDiskInBytes] Region size on disk in bytes.

  /// [sizeOnNetworkInBytes] Region size, for downloading/during network operations, in bytes. Regions are downloaded in
  /// compressed form and hence they have reduced size on network.

  /// [childRegions] All child regions for current region.
  /// Note that each child can again contain multiple children.
  /// A downloadable region will contain the content of all children.

  Region._(this.regionId, this._parentId, this.name, this.sizeOnDiskInBytes, this.sizeOnNetworkInBytes, this.childRegions);
  Region(RegionId regionId)
    : regionId = regionId, _parentId = RegionId(0), name = "", sizeOnDiskInBytes = 0, sizeOnNetworkInBytes = 0, childRegions = null;
  @override
...
Pointer<Void> sdkMaploaderRegionToFfi(Region value) {
  ...
  final _parentIdHandle = sdkMaploaderRegionidToFfi(value._parentId);
