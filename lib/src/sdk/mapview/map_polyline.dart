...
abstract class MapPolyline {
  ...
  ///
  /// Note: After creating [MapPolyline] with this representation, the deprecated [MapPolyline]
  /// properties, do not work and any change to them will be ignored. Any modifications to polyline
  ...
  /// `mapContentCategoriesToBlock`, works and any change to them will be ignored.
  /// Further modifications to polyline appearance must be done with subsequent calls to
  /// `setRepresentation`.
  ///
  ...
  ///
  /// [animation] The animation to start. This object may be shared between multiple instances of map
  /// polyline. Starting animation on one polyline does not influence any ongoing animations on
  /// other polylines.
  ///
  ...
  /// Gets the draw order of the polyline. Polylines with a higher draw order are drawn on top
  ...
  /// For [DrawOrderType.mapSceneAdditionOrderDependent] map polylines with outlines with
  ...
  /// outlines with the same draw order all outlines are rendered first in an arbitrary order
  ...
  /// to a map scene. There is no possibility that parts of another polyline, regardless of
...
/// Represents a dash pattern for map polyline consisting of images rendered with certain gaps
/// from each other.
///
/// This dash pattern representation consists only of images rendered at certain
/// points along polyline. For rendering them without any distortions, polyline gets sliced into
...
abstract class MapPolylineDashImageRepresentation implements MapPolylineRepresentation {
  ...
  /// Only map measure values in range \[1-20\]
  /// are supported. Length values itself must be positive.
  /// For intermediate values between provided map measures, linear
  ...
  /// can be provided representing the same integer map measure.
  /// Only map measure values in range \[1-20\]
  /// are supported. Width values itself must be positive.
  /// For intermediate values between provided map measures, linear
  /// interpolation will be used.
  /// For `measureKind` property only [MapMeasureKind.zoomLevel] is supported.
  /// For `sizeUnit``property` only [RenderSizeUnit.meters] is supported.
  ...
  /// can be provided representing the same integer map measure.
  /// Only map measure values in range \[1-20\]
  /// are supported. Length values itself must not be negative.
  /// For intermediate values between provided map measures, linear
  /// interpolation will be used.
  /// For `measureKind` property only [MapMeasureKind.zoomLevel] is supported.
  /// For `sizeUnit``property` only [RenderSizeUnit.meters] is supported.
